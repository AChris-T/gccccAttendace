...
const FeedbackItem = ({ feedback }) => {
    ...
    const creatorName = feedback.created_by.full_name ||
    ...
    return (
        <div className="bg-linear-to-br from-gray-50 to-gray-50 dark:from-gray-700/50 dark:to-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-600 hover:shadow-md transition-shadow duration-200">
            <div className="flex flex-row sm:items-start justify-between gap-2 mb-3">
                ...
                <span className="flex items-center text-xs text-gray-500 dark:text-gray-400 italic">
    ...
    <div>
        <Badge size="sm" color="error">
            <span className="capitalize">
                {person?.attendance?.status}: {formatFullDateTime(person?.attendance_date)}
            </span>
        </Badge>
    </div>
);
...
export const FollowupCard = ({ person, activeTab }) => {
    ...
    return (
        <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow transition-all duration-300 border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="min-w-0 truncate">
                            <Link
                                to={profileLink}
                                target="_blank"
                                className="flex gap-3 text-lg hover:underline sm:text-xl font-bold text-gray-900 dark:text-white mb-1"
                            >
                                {person.full_name}
                            </Link>
            ...
            {/* Collapsible Content */}
            <div
                className={`absolute left-0 right-0 top-full bg-white dark:bg-gray-800 rounded-b-xl border-t border-gray-200 dark:border-gray-700 shadow-lg transition-all duration-500 ease-in-out overflow-hidden ${isOpen
