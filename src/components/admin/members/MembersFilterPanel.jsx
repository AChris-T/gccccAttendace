...
const MembersFilterPanel = ({
    ...
    const {
        ...
        resolver: yupResolver(filterMembersSchema),
        defaultValues: {
            date_of_birth: initialFilters?.date_of_birth || [],
            birth_month: initialFilters?.birth_month || null,
            community: initialFilters?.community || null,
        }
    });
    ...
    useEffect(() => {
        if (initialFilters) {
            ...
            // Set filter mode based on which field has data
            if (initialFilters.birth_month) {
                setFilterByMonth(true);
            } else if (initialFilters.date_of_birth?.length > 0) {
                setFilterByMonth(false);
            }
        }
    ...
    const handleApply = handleSubmit((data) => {
        const processedData = {
            ...
            ...(!filterByMonth
                ? { date_of_birth: data.date_of_birth || [], birth_month: null }
                : { birth_month: data.birth_month || null, date_of_birth: [] }
            )
        };
        onApply?.(processedData);
    });
    ...
    const handleReset = useCallback(() => {
        const resetData = INITIAL_FILTERS;
        reset(resetData);
        setFilterByMonth(false);
        onReset?.(resetData);
    }, [reset, onReset]);
            ...
            <form onSubmit={handleApply}>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="p-5 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 rounded-xl border border-blue-100 dark:border-blue-900/30">
                        ...
                        {/* Mode Description */}
                        <div className="mb-4 flex items-start gap-2.5 text-xs text-gray-700 dark:text-gray-300 bg-white/60 dark:bg-gray-900/40 p-3.5 rounded-lg border border-blue-200/50 dark:border-blue-800/30 backdrop-blur-sm">
                            <InfoIcon className="w-4 h-4 mt-0.5 flex-shrink-0 text-blue-600 dark:text-blue-400" />
                            <p className="leading-relaxed">
                                {filterByMonth
                                    ? 'Filter members by their birth month. Perfect for planning monthly birthday celebrations, sending group wishes, or organizing birth month events and gatherings.'
