...
const AssignedMembers = ({ assignedMembers }) => {
    ...
    return (
        <>
            <div className="mt-7 bg-white p-5 shadow overflow-x-auto">
                <table className="w-full whitespace-nowrap">
                    <tbody>
                        {assignedMembers?.map((assignedMember, i) => <>
                            <tr key={assignedMember.id} tabindex="0" className="focus:outline-none h-16 border border-gray-100 rounded">
                                <td className="">
                                    <div className="flex items-center pl-5">
                                        <Link target='_blank' to={`/members/${assignedMember.id}`} className="text-base font-medium leading-none text-gray-700 mr-2"> {assignedMember.name}</Link>
                                ...
                                <td className="pl-24">
                                    <div className="flex items-center">
                                        ...
                                        <p className="text-sm leading-none text-gray-600 ml-2">{assignedMember?.email}</p>
                                    </div>
                                </td>
                                <td className="pl-5">
                                    <div className="flex items-center">
                                        ...
                                        <p className="text-sm leading-none text-gray-600 ml-2">{assignedMember?.phone}</p>
                                    </div>
                                </td>
                                <td className="pl-5">
                                    <div className="flex items-center">
                                        ...
                                        <p className="text-sm leading-none text-gray-600 ml-2">{assignedMember?.gender}</p>
                                    </div>
                                </td>
                            </tr>
                            {/* <tr key={Math.random()} className="h-3"></tr> */}
                        </>)}
                    </tbody>
                </table>
            </div>
