...
export default function useEventCalculatorState({
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetEvent(eventKey ? { event: eventKey } : {}, {
  ...
  }, [existingRegistration, dates, hasLoadedExisting, saved]);
  ...
  const handleProceedToPay = useCallback(async () => {
    ...
    if (!registrationData) {
      Toast.error(
        'Please save your registration before proceeding to payment.'
      );
      return;
    }
    ...
    if (!registrationId) {
      Toast.error(
        'Unable to locate your registration record. Please save again.'
      );
      return;
    }
